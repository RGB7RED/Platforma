...
- Исправлен импорт `Todo` в `api/routes.py`, чтобы типы были определены для эндпоинтов.
- Добавлены экспортируемые функции в `services/todo_service.py`, которые импортируют тесты и роуты.
- В `repositories/todo_repository.py` обеспечен экспорт класса `TodoRepository`, ожидаемого тестами.
...
3. `AIOrchestrator` запускает роли `researcher`, `designer`, `planner`, `coder`, `reviewer`.
...
2. **design** → `AIDesigner`
3. **planning** → `AIPlanner`
4. **implementation** → `AICoder`
5. **review** → `AIReviewer`

Инвариант: стадия **design** всегда выполняется только после получения требований (**research**),
а планировщик и оркестратор гарантируют наличие `requirements` перед запуском дизайна.
После **design** формируется **planning**, где строится пошаговый план реализации.
Если финальное ревью не прошло, оркестратор возвращается на **planning** для обновления плана,
после чего повторяется **implementation** и **review** (до лимита циклов).
...
- **Planner**
  - Цель: построить пошаговый план реализации.
  - Ограничения: не меняет требования и архитектуру.
  - Артефакты: `implementation_plan` с шагами и проверками.
- **Reviewer**
